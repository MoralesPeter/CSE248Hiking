...
public class EditUserController {
    ...
    public void editUser(ActionEvent event) throws FileNotFoundException, IOException {
    	...
    	if(userStorage.containsUser(tfUsername.getText())) {
    		if(tfPassword.getText().equals(tfConfirmPassword.getText())) {
    			if(tfPhoneNumber.getText().length() == 10 && tfPhoneNumber.getText().matches("^[0-9]+$")) {
    				User editing = userStorage.searchUser(tfUsername.getText());
    				editing.setPassword(tfPassword.getText());
    				editing.setFirstName(tfFirstName.getText());
    				editing.setLastName(tfLastName.getText());
    				editing.setPhoneNumber(Long.parseLong(tfPhoneNumber.getText()));
    				editing.setImage(userImage);
    				userStorage.save();
    				openTrailView();
    			} else {
